...
% Define the mel frequency scale parameters
f0 = 700 / fs;
fn2 = floor(n/2);

% Compute the mel filter spacing
lr = log(1 + 0.5/f0) / (p+1);

% Convert mel frequencies to FFT bin numbers
bl = n * (f0 * (exp([0 1 p p+1] * lr) - 1));

% Define the bin ranges for the filters
b1 = floor(bl(1)) + 1;
b2 = ceil(bl(2));
b3 = floor(bl(3));
